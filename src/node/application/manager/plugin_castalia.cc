...
static ByteStreamReader *network_get_apdu_stream(Context *ctx)
{
	...
	if (buffer_retry) {
		...
		write(sk, m_st_msg.recv_str.c_str(), strlen(m_st_msg.recv_str.c_str()));
 ...
 */
int plugin_network_castalia_manager_setup(CommunicationPlugin *plugin, int pport)
{
