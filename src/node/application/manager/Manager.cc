...
void Manager::startup()
{
	...
	/*total number of plugins, the manger has one 
	 * plugin for each agent. The number of each plugin
	 * in manager HAS to be only even numbers.
	 * */
	...
	* map variables from plugin_castalia.cc file*/
	...
	dataSN.clear(); //data sequence number
	last_packet.clear();// control duplicate packets
...
void Manager::timerFiredCallback(int index)
{
	...
	//free(m_st_msg[index].buff_msg);
	//m_st_msg[index].buff_msg = NULL;
